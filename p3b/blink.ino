...
Kinda sorta work. "This mode achieves the lowest power consumption while retaining the contents of SRAM and registers."
Problem is the sampling period cannot be guaranteed to be 1s since it now needs to wait for sensor's response.
Don't deploy this.
...
#define N_GROUP (1)  // make sure the total message length stay under 255 chars!
...
void clocksync() {
  if (!Particle.syncTimePending()) {
    Serial1.println("clocksync() begin");
    Particle.connect();
    Particle.syncTime();
    waitUntil(Particle.syncTimeDone);
    Serial1.println("clocksync() end");
  } else {
    Serial1.println("clocksync() in progress");
  }
}
...
Serial1.println(readings_i);
  ...
  for (int i = 0; i < 10; i++) {
    ...
    //    Serial.println(r);
      ...
      delay(30);  // 6x30 = 180ms > sensor response time?
    }
  }
  ...
  System.sleep(D4,RISING,1);
  Serial1.println("ohayou");
}
